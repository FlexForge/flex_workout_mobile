...
class NormalSetInputForm extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return NormalSetFormBuilder(
      model: NormalSet(
        load: setType.normalSet?.load,
        reps: setType.normalSet?.reps,
        units: setType.normalSet?.units.index ?? 0,
      ),
      builder: (context, form, child) => Column(
        ...
        children: [
          ...
          ReactiveNormalSetFormConsumer(
            builder: (context, form, child) {
              return Text(
                  '${setType.sectionIndex} ${setType.organizerIndex} ${setType.setIndex}');
            },
          ),
          const SizedBox(height: AppLayout.p4),
        ],
